...
# a "bad" control string is produced e.g. aaaxbbbbyyhwawiwjjjwwm with letters not
# from a to m.
...
def printer_error(letters):
    return f"{len(''.join(letter for letter in letters if letter > 'm'))}/{len(letters)}"


# letters = "aaaxbbbbyyhwawiwjjjwwm"

# print(len(''.join(letter for letter in letters if letter > 'm')))

print(printer_error("aaabbbbhaijjjm"))
print(printer_error("aaaxbbbbyyhwawiwjjjwwm"))
print(printer_error("aaabbbdddyuwerryyuuzz"))
